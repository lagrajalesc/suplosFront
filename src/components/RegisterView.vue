...
<div class="body-content">
  <div class="module">
    ...
    <div class="form" action="form.php" method="post" enctype="multipart/form-data" autocomplete="off">
      <div class="alert alert-error"></div>
      <input type="text" placeholder="Nombre" v-model="firstName" required />
      <input type="text" placeholder="Apellido" v-model="lastName" required />
      <input type="email" placeholder="Email" v-model="email" required />
      <input type="password" placeholder="Password" v-model="password" autocomplete="new-password" required />
      <input type="submit" value="Register" name="register" @click="register()" class="btn btn-block btn-primary" />
    </div>
  </div>
</div>
</template>
...
export default {
   // se crean los datos y se recolectan los datos necesarios para la creación de un nuevo usuario
    data() {
        return {
    ...
    methods: {
        register() {
          // se valida que se capturen los datos necesarios para el funcionamiento de la pi
          if(!this.firstName || !this.lastName || !this.email || !this.password){
            alert("Debe diligenciar todos los campos")
              ...
              // se capturan los datos del formulario, se asignana a variables y se envían como cuerpo de la solicitud
              axios
              ...
              // se da respuesta de acuerdo a lo que retorne la api
              .then((response) => {
                    var error = response.data.error
                    if(error){
                      alert(response.data.message)
                    }else{
                      // en caso de que la api se consuma correctamente, se redirecciona hacia otra vista
                      location.href = "http://localhost:8080/#/accountCreated";
                    }
              });
          }
